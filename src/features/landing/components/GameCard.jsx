...
const GameCard = ({ game }) => {
  const defaultCoverArtUrl =
    'https://via.placeholder.com/600x450/CCCCCC/FFFFFF?text=No+Image';
  const storeUrl =
  ...
  const genreOne =
    Array.isArray(game?.genre) && game.genre.length ? game.genre[0] : null;
  ...
  const showPopular =
    ...
    (Boolean(game?.isPopular) ||
      (typeof game?.popularityScore === 'number' &&
        game.popularityScore >= 20));
