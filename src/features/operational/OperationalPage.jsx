    ...
    try {
      ...
      shiftSnapshot.docs.forEach(docItem => {
        ...
        if (!dailyAdminAggregates[groupKey]) {
          dailyAdminAggregates[groupKey] = { date: startTime, adminName: d.adminName, totalDurationHours: 0, totalGrossIncome: 0, totalOrders: 0 };
        }
      ...
      // Aggregate recap
      const totalAdminPay = calculatedShiftReport.reduce((sum, r) => sum + r.pay, 0);
      const totalGrossRevenue = calculatedRevenueReport.reduce((sum, r) => sum + (r.grossIncome || 0), 0);
      const totalNetRevenue = calculatedRevenueReport.reduce((sum, r) => sum + (r.calculatedNetRevenue || 0), 0);
      const totalSuccessfulOrders = calculatedRevenueReport.reduce((sum, r) => sum + (r.successfulOrders || 0), 0);
      const totalAdSpend = calculatedRevenueReport.reduce((sum, r) => sum + (r.adSpend || 0), 0);
      ...
      setRecapData({
        totalGrossRevenue, totalAdminPay, totalNetRevenue, netProfit, salaryPercentage, avgRevenuePerOrder, totalSuccessfulOrders, totalAdSpend
      });
    } catch (error) {
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="overflow-x-auto no-scrollbar">
        <div className="inline-flex bg-white rounded-lg shadow p-1">
          <button onClick={() => setActiveTab('dailyRecap')} className={`px-4 py-2 rounded-md ${activeTab === 'dailyRecap' ? 'bg-blue-600 text-white' : 'text-gray-700'}`}>Daily Recap</button>
          <button onClick={() => setActiveTab('adminShift')} className={`ml-2 px-4 py-2 rounded-md ${activeTab === 'adminShift' ? 'bg-blue-600 text-white' : 'text-gray-700'}`}>Admin Shift</button>
          <button onClick={() => setActiveTab('netRevenue')} className={`ml-2 px-4 py-2 rounded-md ${activeTab === 'netRevenue' ? 'bg-blue-600 text-white' : 'text-gray-700'}`}>Net Revenue</button>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <div className="flex flex-wrap gap-3">
          {['today','yesterday','thisMonth','lastMonth'].map((p) => (
            ...
            >
              {p === 'today' ? 'Hari Ini' : p === 'yesterday' ? 'Kemarin' : p === 'thisMonth' ? 'Bulan Ini' : 'Bulan Lalu'}
            </button>
          ))}
          <button onClick={() => setShowManualDateRange(!showManualDateRange)} className="px-3 py-2 rounded-md bg-gray-100 text-gray-700">Pilih Tanggal Manual</button>
        </div>
        ...
        {showManualDateRange && (
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
            <div>
              ...
              <DatePicker selected={startDate} onChange={(d) => setStartDate(d)} className="mt-1 p-2 w-full border rounded-md" />
            </div>
            <div>
              <label className="text-sm text-gray-600">Tanggal Selesai</label>
              <DatePicker selected={endDate} onChange={(d) => setEndDate(d)} className="mt-1 p-2 w-full border rounded-md" />
            </div>
            <div>
              <button onClick={() => fetchDataForPeriod(startDate, endDate)} className="w-full bg-indigo-600 text-white px-3 py-2 rounded-md">Tampilkan</button>
            </div>
          </div>
        )}
      </div>
