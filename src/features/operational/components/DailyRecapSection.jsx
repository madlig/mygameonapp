...
const StatCard = ({
  ...
  <div className="bg-white p-4 rounded-lg shadow flex flex-col justify-between">
    ...
    <p className="mt-3 text-2xl font-bold text-gray-900">
      {isCurrency
        ? `${currency} ${value.toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`
        : value}
    </p>
    ...
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          extraInfo="Sesuai angka di aplikasi Shopee"
        />
        <StatCard
          title="Adjusted Gross"
          value={totalAdjustedGross}
          extraInfo="Gross Shopee - cancel/return"
        />
        <StatCard title="Total Biaya Iklan" value={totalAdSpend} />
        <StatCard
          title="Total Pemasukan Bersih"
          value={totalNetRevenue}
          extraInfo="Setelah biaya Shopee & iklan, sebelum gaji"
        />
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="text-sm text-gray-600">
            <div className="text-xs text-gray-500">Total Pesanan Sukses</div>
            <div className="font-medium">{totalSuccessfulOrders || 0}</div>
          </div>
          <div className="text-sm text-gray-600">
            <div className="text-xs text-gray-500">Avg Revenue / Order</div>
            <div className="font-medium">
              Rp{' '}
              {avgRevenuePerOrder.toLocaleString('id-ID', {
          ...
          <div className="text-sm text-gray-600">
            ...
            <div className="font-medium">
              {salaryPercentage ? `${salaryPercentage.toFixed(1)}%` : '—'}
            </div>
          </div>
          <div className="text-sm text-gray-600">
            <div className="text-xs text-gray-500">Total Gaji Admin</div>
            <div className="font-medium">
              Rp {totalAdminPay.toLocaleString('id-ID')}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <h3 className="text-md font-semibold text-gray-700 mb-3">
          Pendapatan Pribadi (Owner)
        </h3>
        <div className="text-sm text-gray-600">
          Akan diisi setelah konfigurasi pemisahan akun owner dan admin.
        </div>
      </div>
    </section>
  );
