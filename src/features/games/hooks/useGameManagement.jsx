...
import { collection, query, onSnapshot, orderBy } from 'firebase/firestore'; // Import onSnapshot
...
export const useGameManagement = () => {
  ...
  useEffect(() => {
    ...
    const minVal =
      minRaw !== '' && minRaw !== null
        ? parseFloat(String(minRaw).replace(',', '.'))
        : null;
    const maxVal =
      maxRaw !== '' && maxRaw !== null
    ...
    if (
      ...
      processedData = processedData.filter((g) => {
        ...
        const sizeInMB = parseSizeToMB(
          g.size ?? g.sizeInNumber ?? null,
          fallbackUnit
        );
    ...
    const comparator = (a, b) => {
      ...
      const getVal = (item) => {
        ...
        if (key === 'dateAdded')
          return parseToDate(item.dateAdded)?.getTime() || -Infinity;
  ...
  return {
    ...
    handleSizeInputChange: (key, val) =>
      setFilters((p) => ({ ...p, size: { ...p.size, [key]: val } })),
    handleDateInputChange: (key, val) =>
      setFilters((p) => ({ ...p, dateAdded: { ...p.dateAdded, [key]: val } })),
    toggleRowSelection,
    parseSizeToMB,
  };
};
