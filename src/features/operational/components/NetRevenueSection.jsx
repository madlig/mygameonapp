...
const NetRevenueSection = ({ revenueReport, onRefreshRequest }) => {
  ...
  try {
    ...
    stagedUpdates.forEach(updatedReport => {
      ...
      if (originalReport && 
          (originalReport.voucherCost !== updatedVoucherCost || originalReport.adSpend !== updatedAdSpend)) 
      {
        changesCount++;
      ...
      <div className="mt-6">
        ...
        {activeInputTab === 'biaya' && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-800">Input Biaya untuk Periode yang Ditampilkan</h3>
            <div className="overflow-x-auto border rounded-md">
              <table className="min-w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tanggal</th>
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Biaya Voucher (Rp)</th>
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Biaya Iklan (Rp)</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {stagedUpdates.length > 0 ? stagedUpdates.map((report) => (
                    <tr key={report.id}>
                      <td className="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-800">
                        {report.date.toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap">
                        <input
                          type="number"
                          value={report.voucherCost || ''}
                          onChange={(e) => handleStagedUpdateChange(report.id, 'voucherCost', e.target.value)}
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">
                        <input
                          ...
                          onChange={(e) => handleStagedUpdateChange(report.id, 'adSpend', e.target.value)}
                          className="w-full p-1 border rounded-md"
                          placeholder="0"
                        />
                      </td>
                    ...
                    <tr><td colSpan="3" className="text-center py-4 text-gray-500">Pilih rentang tanggal di atas untuk menampilkan data.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            {stagedUpdates.length > 0 && (
              <button onClick={handleBatchUpdate} className="w-full bg-blue-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-blue-700">
                ...
                <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-full overflow-y-auto">
                  ...
                  <form onSubmit={handleUpdateReport} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <label htmlFor="editGrossIncome" className="block text-sm font-medium text-gray-700">Pendapatan Kotor</label>
                        <input type="number" id="editGrossIncome" value={editingReport.grossIncome || ''} onChange={(e) => setEditingReport({ ...editingReport, grossIncome: e.target.value })} className="mt-1 p-2 w-full border rounded-md" />
                      </div>
                      <div>
                        <label htmlFor="editTotalOrders" className="block text-sm font-medium text-gray-700">Total Pesanan</label>
                        <input type="number" id="editTotalOrders" value={editingReport.totalOrders || ''} onChange={(e) => setEditingReport({ ...editingReport, totalOrders: e.target.value })} className="mt-1 p-2 w-full border rounded-md" />
                      </div>
                      <div>
                        <label htmlFor="editVoucherCost" className="block text-sm font-medium text-gray-700">Voucher Penjual</label>
                        <input type="number" id="editVoucherCost" value={editingReport.voucherCost || ''} onChange={(e) => setEditingReport({ ...editingReport, voucherCost: e.target.value })} className="mt-1 p-2 w-full border rounded-md" />
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label htmlFor="editCanceledOrders" className="block text-sm font-medium text-gray-700">Pesanan Dibatalkan</label>
                        <input type="number" id="editCanceledOrders" value={editingReport.canceledOrders || ''} onChange={(e) => setEditingReport({ ...editingReport, canceledOrders: e.target.value })} className="mt-1 p-2 w-full border rounded-md" />
                      </div>
                      <div>
                        <label htmlFor="editCanceledValue" className="block text-sm font-medium text-gray-700">Nilai Batal (Rp)</label>
                        <input type="number" id="editCanceledValue" value={editingReport.canceledValue || ''} onChange={(e) => setEditingReport({ ...editingReport, canceledValue: e.target.value })} className="mt-1 p-2 w-full border rounded-md" />
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label htmlFor="editReturnedOrders" className="block text-sm font-medium text-gray-700">Pesanan Dikembalikan</label>
                        <input type="number" id="editReturnedOrders" value={editingReport.returnedOrders || ''} onChange={(e) => setEditingReport({ ...editingReport, returnedOrders: e.target.value })} className="mt-1 p-2 w-full border rounded-md" />
                      </div>
                      <div>
                        <label htmlFor="editReturnedValue" className="block text-sm font-medium text-gray-700">Nilai Kembali (Rp)</label>
                        <input type="number" id="editReturnedValue" value={editingReport.returnedValue || ''} onChange={(e) => setEditingReport({ ...editingReport, returnedValue: e.target.value })} className="mt-1 p-2 w-full border rounded-md" />
                      </div>
                    </div>
                    
                    <div>
                      <label htmlFor="editAdSpend" className="block text-sm font-medium text-gray-700">Biaya Iklan (Rp)</label>
                      <input type="number" id="editAdSpend" value={editingReport.adSpend || ''} onChange={(e) => setEditingReport({ ...editingReport, adSpend: e.target.value })} className="mt-1 p-2 w-full border rounded-md" />
                    </div>

                    <div className="mt-6 flex justify-end space-x-3">
                      <button type="button" onClick={() => setIsEditModalOpen(false)} className="bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300">
                        Batal
                      </button>
