    ...
    <section className="space-y-6">
      <div className="bg-white p-4 rounded-lg shadow">
        ...
        {activeShift && (
          <div className="mt-4 p-3 bg-gray-50 rounded">
            <p className="text-sm">
              Shift Aktif:{' '}
              <span className="font-medium">{activeShift.adminName}</span>
            </p>
            <p className="text-sm">
              Mulai:{' '}
              <span className="font-medium">
                {activeShift.startTime?.toLocaleTimeString?.() || ''}
              </span>
            </p>
            <p className="text-sm">
              Durasi:{' '}
              <span className="font-medium">{getActiveShiftDuration()}</span>
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <h3 className="text-md font-semibold text-gray-700 mb-2">
          Laporan Gaji per Tanggal & Admin
        </h3>
              ...
              {shiftReport.length === 0 ? (
                ...
                shiftReport.map((report, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-sm">
                      {report.totalDurationHours?.toFixed(2)} jam
                    </td>
                    <td className="px-3 py-2 text-sm">
                      Rp {report.totalGrossIncome?.toLocaleString('id-ID')}
                    </td>
                    <td className="px-3 py-2 text-sm">
                      {report.totalOrders || 0}
                    </td>
                    <td className="px-3 py-2 text-sm font-bold text-purple-700">
                      Rp {report.pay?.toLocaleString('id-ID')}
                    </td>
                  </tr>
                ))
              )}
