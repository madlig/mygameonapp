...
const TaskPage = () => {
  ...
  useEffect(() => {
    ...
    if (selectedProjectId) {
      ...
      currentQuery = query(tasksCollectionRef, orderBy('createdAt', 'desc'));
    }
  ...
  const handleSaveProject = async (projectName, projectDescription) => {
    ...
    try {
      if (editingProject) {
        await updateProject(userId, editingProject.id, {
          name: projectName,
  ...
  const handleDeleteProject = async (projectId) => {
    if (
      window.confirm(
        'Are you sure you want to delete this project and all its associated tasks?'
      )
    ) {
      if (!userId) {
        setError('User not authenticated.');
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tasks.length === 0 ? (
            <p className="text-gray-600 col-span-full">
              No tasks found for this view. Add a new one!
            </p>
          ) : (
            tasks.map((task) => {
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      <p>
                        ...
                        <span className="font-medium text-sm text-purple-600">
                          {task.isDailyTask ? 'Daily Task' : 'Task'}
                        </span>
                      </p>
                      <p>
                        Status:{' '}
                        <span
                          className={`font-medium ${task.status === 'Done' ? 'text-green-600' : task.status === 'In Progress' ? 'text-yellow-600' : 'text-gray-600'}`}
                        >
                          {task.status}
                        </span>
                      </p>
