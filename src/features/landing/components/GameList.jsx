...
const GenreIcon = () => (
  ...
  >
    <path
      ...
      strokeWidth="2"
      d="M7 7h.01M7 3h5a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h2zM7 13h.01M7 17h5a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2h2z"
    ></path>
  </svg>
);
const SizeIcon = () => (
...
const canonicalMap = [
  ...
  {
    test: (k) => k.includes('open') && k.includes('world'),
    label: 'Open World',
  },
  ...
  {
    test: (k) => k.includes('gamepad') || k.includes('controller'),
...
const GameList = ({ hit }) => {
  ...
  const sizeUnit = game.sizeUnit || game.unit;
  const sizeWithUnit =
    game.size && sizeUnit ? `${game.size} ${sizeUnit}` : game.size || null;

  const version =
    typeof game.version === 'string' && game.version.trim()
      ? game.version.trim()
      : null;
  ...
  return (
    <div
      className="
        ...
        /* Mobile: full fluid (no card chrome) */
        border-0 shadow-none rounded-none
        /* Desktop/tablet enhance */
        sm:border sm:border-gray-200 sm:shadow-sm sm:hover:shadow-md sm:rounded-xl sm:overflow-hidden sm:bg-white
        bg-white
      "
    >
      {/* Gambar full width tanpa radius di mobile; radius muncul di sm: */}
      ...
      {/* Konten: mobile pakai px-4 py-4; desktop dapat padding tambahan jika mau */}
      <div className="flex flex-col flex-grow px-4 py-4 sm:px-4 sm:py-4">
        ...
        {/* Tags (tinggi tetap, wrap di desktop) */}
        ...
        {/* Metadata selalu dua baris konsisten */}
