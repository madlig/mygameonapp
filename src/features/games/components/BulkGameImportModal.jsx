...
import { X, UploadCloud, FileSpreadsheet, Loader2, Info } from 'lucide-react';
import {
  writeBatch,
  doc,
  collection,
...
const BulkGameImportModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const mapRowToFirestore = (row) => {
    // A. Judul
    const title =
      row.judul || row.title || row.name || row['nama game'] || 'No Title';
    ...
    const {
      ...
      unit: sizeUnit,
      sizeInMB,
    } = parseSizeRobust(rawSize);

    // C. Genre & Tags
    const rawGenre = row.genre || row.kategori || '';
    const genreArray = rawGenre
      .split(',')
      .map((g) => g.trim())
      .filter((g) => g);
    ...
    else if (upperType.includes('ELAMIGOS'))
      installerType = 'INSTALLER ElAmigos';
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        Swal.fire({
          ...
          text: `Berhasil mengimpor ${allData.length} game.`,
          icon: 'success',
        });
        onSuccess();
        onClose();
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <h3 className="font-bold text-slate-800 flex items-center gap-2">
            <UploadCloud size={20} className="text-blue-600" /> Import Game dari
            CSV
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 text-sm text-blue-800">
            <div className="flex items-center gap-2 font-bold mb-2">
              <Info size={16} /> Format Kolom CSV
            </div>
            <p className="mb-2">
              Gunakan baris pertama sebagai Header. Format Size fleksibel
              (contoh: &quot;45 GB&quot;, &quot;500MB&quot;, &quot;429,5
              MB&quot;).
            </p>
                      ...
                      <thead className="bg-white border-b border-slate-100">
                        <tr>
                          {Object.keys(previewData[0]).map((key) => (
                            <th
                              key={key}
                              className="p-2 font-medium text-slate-500 uppercase whitespace-nowrap"
                            >
                              {key}
                            </th>
                          ))}
                        ...
                        {previewData.map((row, idx) => (
                          <tr key={idx}>
                            {Object.values(row).map((val, i) => (
                              <td
                                key={i}
                                className="p-2 text-slate-700 truncate max-w-[150px]"
                              >
                                {val}
                              </td>
                            ))}
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
          <button
            onClick={onClose}
            disabled={isProcessing}
            className="px-4 py-2 border border-slate-300 text-slate-700 rounded-lg text-sm font-medium hover:bg-white transition-colors"
          >
            Batal
          </button>
          <button
            ...
            className="px-6 py-2 bg-slate-900 text-white rounded-lg text-sm font-bold hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center shadow-lg active:scale-95 transition-all"
          >
            {isProcessing ? (
              <>
