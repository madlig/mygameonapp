...
import { ArrowLeft, Send, CheckCircle, AlertCircle, Loader2, ShoppingBag, ThumbsUp, Info, Clock } from 'lucide-react';
...
const RequestGamePage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (lastRequestTime && Date.now() - parseInt(lastRequestTime) < COOLDOWN_PERIOD) {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        if (currentUser) {
          ...
          const cleanShopee = data.shopeeUsername.replace(/[^a-zA-Z0-9_.-]/g, '');
    ...
    <div className="min-h-screen bg-slate-50">
      <nav className="bg-white border-b border-slate-200 px-4 py-3 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
            <Link to="/" className="flex items-center text-slate-500 hover:text-slate-800 transition-colors font-medium">
                <ArrowLeft size={20} className="mr-2" />
                Kembali ke Beranda
            </Link>
            <span className="text-slate-400 text-sm font-medium">MyGameOn Request</span>
        </div>
      </nav>
      ...
      <main className="max-w-xl mx-auto px-4 py-12">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-8 border-b border-slate-100 bg-gradient-to-r from-blue-600 to-indigo-700 text-white">
                ...
                <p className="text-blue-100">Isi form di bawah untuk request game PC via Shopee.</p>
            </div>
            ...
            <div className="p-8">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-8 flex items-start gap-3">
                    ...
                    <div className="text-sm text-blue-800">
                        ...
                        <ul className="list-disc list-inside space-y-1 opacity-90 text-xs sm:text-sm">
                            <li>Pastikan penulisan <b>Judul Game</b> benar dan tidak typo.</li>
                            <li>Gunakan nama lengkap (contoh: <i>"Resident Evil 4"</i> bukan <i>"RE4"</i>).</li>
                            <li>Request dengan nama yang salah/typo berpotensi <b>ditolak</b> oleh Admin.</li>
                        </ul>
                    </div>
                </div>
                ...
                {submitStatus === 'success' && (
                    <div className="mb-6 p-4 bg-green-50 text-green-700 rounded-xl flex items-center animate-in fade-in slide-in-from-top-4">
                        ...
                        <div><p className="font-semibold">Permintaan Terkirim!</p><p className="text-sm opacity-90">Terima kasih! Pantau notifikasi Shopee kamu ya.</p></div>
                    </div>
                )}
                ...
                {submitStatus === 'voted' && (
                    <div className="mb-6 p-4 bg-blue-50 text-blue-700 rounded-xl flex items-center animate-in fade-in slide-in-from-top-4">
                        ...
                        <div>
                            <p className="font-semibold">Request Ditambahkan ke Antrian!</p>
                            <p className="text-sm opacity-90">Game ini sudah ada di daftar request. Kami menambahkan +1 vote dukunganmu.</p>
                        </div>
                    </div>
                )}
                ...
                {submitStatus === 'rate_limit' && (
                    <div className="mb-6 p-4 bg-amber-50 text-amber-700 rounded-xl flex items-center animate-in fade-in slide-in-from-top-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm opacity-90">Kamu baru saja mengirim request. Mohon tunggu 2 menit sebelum mengirim lagi.</p>
                        </div>
                    </div>
                )}
                ...
                {submitStatus === 'error' && (
                    <div className="mb-6 p-4 bg-red-50 text-red-700 rounded-xl flex items-center animate-in fade-in slide-in-from-top-4">
                        ...
                        <div><p className="font-semibold">Gagal Mengirim</p><p className="text-sm opacity-90">Terjadi kesalahan koneksi. Coba lagi nanti.</p></div>
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Username Shopee *</label>
                        <div className="relative">
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><ShoppingBag className="h-5 w-5 text-slate-400" /></div>
                            <input 
                                type="text" 
                                {...register("shopeeUsername", { 
                                    required: "Username Shopee wajib diisi",
                                    maxLength: { value: 30, message: "Username terlalu panjang (maks 30)" },
                                    pattern: { value: /^[a-zA-Z0-9_.\-]+$/, message: "Username mengandung karakter tidak valid" }
                                })} 
                        ...
                        {errors.shopeeUsername && <span className="text-red-500 text-sm mt-1">{errors.shopeeUsername.message}</span>}
                        <p className="text-xs text-slate-500 mt-1">Kami akan menghubungi via Shopee jika game tersedia.</p>
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Judul Game PC yang Dicari *</label>
                        <input 
                            type="text" 
                            {...register("gameTitle", { 
                                required: "Judul game wajib diisi",
                                minLength: { value: 3, message: "Judul terlalu pendek" },
                                maxLength: { value: 50, message: "Judul terlalu panjang (maks 100)" }
                            })} 
                        ...
                        />
                        {errors.gameTitle && <span className="text-red-500 text-sm mt-1">{errors.gameTitle.message}</span>}
                    </div>
                    <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-200 active:transform active:scale-[0.98] transition-all flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed">
                        {isSubmitting ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Mengirim...</> : <><Send className="w-5 h-5 mr-2" /> Kirim Request</>}
                    </button>
                </form>
            </div>
        </div>
