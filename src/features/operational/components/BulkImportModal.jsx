...
const validateRow = (row) => {
  ...
  if (!Number.isFinite(gross) || gross < 0)
    errors.push('Invalid Pendapatan Kotor');

  // totalOrders integer
  const orders = Number(row.totalOrders);
  if (!Number.isFinite(orders) || !Number.isInteger(orders) || orders < 0)
    errors.push('Invalid Total Pesanan');

  // optional fields check
  if (row.canceledValue && isNaN(Number(row.canceledValue)))
    warnings.push('Nilai Batal tidak terparse');
  if (row.returnedValue && isNaN(Number(row.returnedValue)))
    warnings.push('Nilai Kembali tidak terparse');
...
const BulkImportModal = ({
  ...
  const handleConfirm = () => {
    ...
    const toImport = rows
      .filter(
        (r) => !excludedIds.has(r.id) && (!r.errors || r.errors.length === 0)
      )
      .map((r) => r.normalized);
    ...
    <Modal onClose={onClose} ariaLabel="Bulk Import Preview">
      <div className="p-6 w-full max-w-4xl">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Periksa hasil parsing. Anda dapat mengedit baris yang bermasalah atau
          mengecualikannya sebelum menyimpan.
        </p>

        <div className="mb-4 flex items-center justify-between">
          <div className="text-sm">
            <strong>{counts.total}</strong> rows —{' '}
            <span className="text-green-600">{counts.valid} valid</span>,{' '}
            <span className="text-yellow-600">{counts.warn} warning</span>,{' '}
            <span className="text-red-600">{counts.err} error</span>
          </div>
          <div className="text-sm">
            <button
              onClick={() => {
            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr
                    key={r.id}
                    className={`${isExcluded ? 'opacity-50' : ''} hover:bg-gray-50`}
                  >
                    <td className="px-3 py-2 text-sm">{r.id + 1}</td>
                    <td className="px-3 py-2 text-sm">
                      {r.normalized.date
                        ? r.normalized.date.toLocaleDateString('id-ID')
                        : '-'}
                    </td>
                    <td className="px-3 py-2 text-sm">
                      Rp {fmt(r.normalized.grossIncome)}
                    </td>
                    <td className="px-3 py-2 text-sm">
                      {r.normalized.totalOrders}
                    </td>
                    <td className="px-3 py-2 text-sm">
                      {status}
                      {r.warnings && r.warnings.length > 0 && (
                    ...
                    <td className="px-3 py-2 text-sm">
                      ...
                      <button
                        ...
                        className={`mr-2 ${isExcluded ? 'text-green-600' : 'text-gray-600'}`}
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  value={
                    editingRow.date
                      ? new Date(editingRow.date).toISOString().split('T')[0]
                      : ''
                  }
                  onChange={(e) =>
