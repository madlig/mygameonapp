...
const FilterGenre = ({ genres = [], selectedGenres = [], onChange }) => {
  return (
    <div className="space-y-2">
      {Array.isArray(genres) && genres.map((genre) => ( // <<<<<< Baris 6 ini kemungkinan besar penyebabnya
        <label key={genre} className="flex items-center space-x-2 text-gray-700">
          <input
            type="checkbox"
            value={genre}
            checked={Array.isArray(selectedGenres) && selectedGenres.includes(genre)}
            onChange={(e) => {
              const isChecked = e.target.checked;
              if (isChecked) {
                // Mengirim array baru langsung ke parent
                onChange([...(Array.isArray(selectedGenres) ? selectedGenres : []), genre]);
              } else {
                // Mengirim array baru langsung ke parent
                onChange((Array.isArray(selectedGenres) ? selectedGenres : []).filter(item => item !== genre));
              }
            }}
